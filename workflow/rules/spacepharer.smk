...
rule spacepharer:
    ...
    threads:
        config["spacepharer"]["threads"]
    params:
        TMPDIR=os.path.join(RESULTS_DIR, "tmp/{mag}/spacepharer"),
    ...
    shell:
        "(date && "
        "spacepharer easy-predict {input.CRISPR} {input.DB} {output} {params.TMPDIR} --threads {threads} --remove-tmp-files --fmt 0 --report-pam 0 && "
        "date) &> {log}"
